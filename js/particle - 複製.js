...
// --- System C: 大腦粒子 (Brain Particle) ---
// ★ 新增：大腦粒子系統變數
let brainParticleSystem;
let brainParticleData = [];
let runBrain = false; // 大腦粒子運作開關
...
const config = {
    ...
    // ★ 新增：大腦粒子設定 (與 First Particle 類似，但初始為散亂狀態)
    brainParticle: {
        count: 60000,
        color: 0x008cff,
        size: 4.4,
...
try {
    ...
    initBrainScrollTrigger(); // ★ 新增：大腦粒子觸發器

    animate();

    console.log("✅ V43 啟動：大腦粒子1初始化與過渡邏輯 | 完美運行");
...
function updateBeam(system) {
    ...
    for (let i = 0; i < cfg.count; i++) {
        ...
        // --- 核心修改：計算散開座標 (Scatter Position) ---
        let scatterX = 0;
        let scatterZ = 0;
        const scatterScale = 2500; // 散開範圍 (大面積)
        const driftSpeed = 0.3; // 緩慢漂浮速度

        // 使用 Math.sin/cos 產生緩慢的大範圍漂浮
        ...
        // --- 核心修改：座標混合 (Linear Interpolation) ---
        // 根據 beamScatterRatio (0~1) 在「路徑座標」與「散開座標」之間進行混合
        // ratio = 0 (完全綁定), ratio = 1 (完全散開)
        
        positions[i3] = pathX + (scatterX - pathX) * beamScatterRatio;
        positions[i3 + 1] = p.y; // Y 軸保持原有流動邏輯，不進行混合
...
function initBeamScrollTriggers() {
    ...
    // ★★★ 核心修改：漸進式解綁觸發器 ★★★
    // 當 .portfolio-spacer-1 進入視窗指定範圍時，平滑改變 beamScatterRatio
    ScrollTrigger.create({
        trigger: ".portfolio-spacer-1",
        start: "top 320px",    // 開始解綁的位置
        end: "top 100px",     // 完全解綁的位置
        scrub: 0.1,            // 平滑過渡係數
        onUpdate: (self) => {
            // self.progress 會在 0 到 1 之間變化
            beamScatterRatio = self.progress;
        }
    });

    // ★★★ 核心修改：淡出與功能關閉觸發器 ★★★
    // 光粒 4、6 在解綁後 (top -10px) 關閉
    // 光粒 1、3、5 在區間 (-10px 到 -160px) 淡出並關閉
    ScrollTrigger.create({
        trigger: ".portfolio-spacer-1",
        start: "top 80px",    // 接續在解綁動作之後
        end: "top -280px",     // 結束點
        scrub: 0.1,
        onUpdate: (self) => {
            const opacity = 1.0 - self.progress; // 1 -> 0
            ...
            // 處理光粒 4, 6 (解綁後淡出/關閉)
            // 由於此觸發器始於解綁結束(-10px)，此處同步進行淡出，確保視覺平滑
        ...
        onEnterBack: () => {
            ...
            allTargets.forEach(sys => {
                if(sys) {
                    ...
                    sys.geometry.setDrawRange(0, sys.userData.originalCount); // 恢復原始數量
                }
            });
        }
    });
...
// ★★★ 新增：大腦粒子系統初始化 ★★★
function initBrainParticle() {
    const pConfig = config.brainParticle;
    const geometry = new THREE.BufferGeometry();
    const positions = new Float32Array(pConfig.count * 3);
    const colors = new Float32Array(pConfig.count * 3);
    const colorObj = new THREE.Color();
    ...
    for (let i = 0; i < pConfig.count; i++) {
        ...
        // 初始狀態：大面積隨機散佈 (模擬解綁後的漂浮狀態)
        positions[i3] = (Math.random() - 0.5) * pConfig.rangeXY * 2;
        positions[i3 + 1] = (Math.random() - 0.5) * pConfig.rangeXY * 2;
        positions[i3 + 2] = (Math.random() - 0.5) * pConfig.rangeZ * 2;

        // 顏色設定 (複製 First Particle)
        // 暫時隨機亮度
        let lightness = 0.3 + Math.random() * 0.4;
        colorObj.setHSL(0.6, 1.0, lightness);
        colors[i3] = colorObj.r;
        colors[i3 + 1] = colorObj.g;
        colors[i3 + 2] = colorObj.b;
    ...
    const texture = createGlowingDot(); // 重用第一粒子的光點貼圖
    const material = new THREE.PointsMaterial({
        size: pConfig.size,
        map: texture,
        vertexColors: true,
    ...
    brainParticleSystem = new THREE.Points(geometry, material);
    // 初始不渲染任何粒子
    brainParticleSystem.geometry.setDrawRange(0, 0); 
    scene.add(brainParticleSystem);
}

// ★★★ 新增：大腦粒子動畫更新 (暫時僅漂浮) ★★★
function updateBrainParticle() {
    if (!brainParticleSystem || !runBrain) return;

    const positions = brainParticleSystem.geometry.attributes.position.array;
    const count = brainParticleSystem.geometry.drawRange.count;

    for (let i = 0; i < count; i++) {
        let i3 = i * 3;
        const data = brainParticleData[i];
        
        // 簡單的緩慢漂浮動畫
        positions[i3] = data.initialX + Math.sin(time * 0.5 + data.noiseOffset) * 20;
        positions[i3 + 1] = data.initialY + Math.cos(time * 0.3 + data.noiseOffset) * 20;
        // Z 軸保持
    }

    brainParticleSystem.geometry.attributes.position.needsUpdate = true;
}

// ★★★ 新增：大腦粒子 ScrollTrigger ★★★
function initBrainScrollTrigger() {
    ScrollTrigger.create({
        trigger: ".portfolio-spacer-1",
        start: "top 120px",
        end: "top 100px",
        ...
        onLeaveBack: () => {
            // 回到上方關閉運作
            runBrain = false;
            if(brainParticleSystem) {
                brainParticleSystem.geometry.setDrawRange(0, 0);
                brainParticleSystem.material.opacity = 0;
            }
        },
        onUpdate: (self) => {
            ...
            if(brainParticleSystem) {
                // 透明度 0 -> 1
                brainParticleSystem.material.opacity = p * (config.brainParticle.opacity || 1);
                
                // 數量 0 -> 60000
                const currentCount = Math.floor(p * config.brainParticle.count);
                brainParticleSystem.geometry.setDrawRange(0, currentCount);
            }
        }
    });
}
...
function animate() {
    ...
    // ★ 更新大腦粒子 (僅在開啟時更新)
    if (runBrain) updateBrainParticle();
