...
try {
  ...
  console.log("✅ V32 啟動：雙重變數控制 | Density (數量) + Flow (長度)");
} catch (e) {
  console.error("❌ 錯誤:", e);
}
...
function createBeamSystem(cfg, type) {
  ...
  // ★ 新增 flowRatio 與 densityRatio 變數
  // densityRatio: 控制數量/密度 (0 ~ 1)
  // flowRatio: 控制長度/流動 (0 ~ 1)
  system.userData = { 
      config: cfg, 
...
function updateBeam(system) {
  ...
  for (let i = 0; i < cfg.count; i++) {
    ...
    // A. 數量過濾 (密度)
    // B. 高度過濾 (流動)
    if (i >= visibleCount || p.y < drawLimitY) {
        positions[i3] = 99999;
...
// ==========================================
// 6. 光束的 ScrollTriggers (精準距離控制)
// ==========================================
function initBeamScrollTriggers() {
    // 1. 全域總開關：spacer-1 bottom 距離 top 100px 時開啟
    ...
    // 2. 光粒 1 & 2 (垂直)
    // A. 漸顯 & 密度：Spacer 2 Top 100px -> -200px
    ScrollTrigger.create({
    ...
    // B. 流動 (Flow)：Spacer 2 Top 100px -> Spacer 2 Bottom 0px
    ScrollTrigger.create({
        trigger: ".competencies-spacer-2",
        start: "top 100px",
        end: "bottom top", 
    ...
    // 3. 光粒 3 & 4 (左弧)
    // A. 漸顯 & 密度：Spacer 2 Top 0px -> -340px
    ScrollTrigger.create({
        trigger: ".competencies-spacer-2",
        start: "top 0px",
        end: "top -340px",
    ...
    // B. 流動：Spacer 2 Top -100px -> Spacer 2 Bottom 0px
    ScrollTrigger.create({
        trigger: ".competencies-spacer-2",
        start: "top -100px",
        end: "bottom top",
    ...
    // 4. 光粒 5 & 6 (右弧)
    // A. 漸顯 & 密度：Spacer 2 Top -200px -> -400px
    ScrollTrigger.create({
        trigger: ".competencies-spacer-2",
        start: "top -200px",
        end: "top -400px",
    ...
    // B. 流動：Spacer 2 Top -200px -> Spacer 2 Bottom 0px
    ScrollTrigger.create({
        trigger: ".competencies-spacer-2",
        start: "top -200px",
        end: "bottom top",
